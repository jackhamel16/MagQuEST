...
void Integrator::LLG_RHS::evaluate(const int step) const
{
  ...
  history->array[0][step][0] = pulse_interactions[0]; // Using pulse to set field at particle 1 ("0")
  ...
  history->array[1][step][0] = history_interactions_past[1];
  history->array[0][step][1] = pulse_interactions[0];
  //for(int sol = 0; sol < num_solutions; ++sol) {
    //Eigen::Vector3d sol_vector =
        //Eigen::Vector3d(H_vec[3 * sol], H_vec[3 * sol + 1], H_vec[3 * sol + 2]);
    //history->array[sol][step][0] = history_interactions_past[sol]V
